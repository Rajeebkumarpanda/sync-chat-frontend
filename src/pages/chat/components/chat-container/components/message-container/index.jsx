...
import { GET_ALL_MESSAGES_ROUTE, GET_CHANNEL_MESSAGES_ROUTE, HOST } from "@/utils/Constant";
...
const MessageContainer = () => {
  ...
  const handleDownload = async (file) => {
    ...
    //2nd way to download
    const reponse = await apiClient.get(`${HOST}/${file}`, {
      responseType: "blob",
    ...
    const urlBlob = window.URL.createObjectURL(new Blob([reponse.data]));
    const link = document.createElement("a");
    link.href = urlBlob;
        ...
        {message.sender._id !== userInfo.id ? (
          <div className="flex justify-start items-center gap-3">
            <Avatar className="h-8 w-8  rounded-full overflow-hidden">
              ...
              <AvatarFallback
                className={`uppercase h-8 w-8 border-[1px] text-lg flex justify-center items-center rounded-full ${getColor(
